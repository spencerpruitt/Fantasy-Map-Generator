...
- **File System Access API**: A modern browser capability (`showSaveFilePicker`) that opens the OS File Explorer "Save As" dialog, letting the user choose the folder and filename. Supported only in Chromium-based browsers (Chrome, Edge, Opera, Brave); absent in Firefox and Safari.
- **Save-Location Picker**: The OS File Explorer dialog shown on *Save to Machine* (where supported) so the user picks the destination folder and filename. It opens on **every** save, so the user can overwrite the same file (by re-selecting it) or write a different file (by choosing a new name/folder) each time — instead of the file being dropped silently into the browser's Downloads folder.
- **Downloads Fallback**: The behavior used when the [[File System Access API]] is unavailable (Firefox/Safari): the `.map` file is written to the browser's Downloads folder (via the shared `downloadFile` helper), accompanied by a one-time note explaining that the save-location picker isn't supported.
